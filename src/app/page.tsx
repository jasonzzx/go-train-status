...
// Station-code badge — same shape as LineBadge, coloured with the parent
// line's colour so every station under a line reads as belonging to it.
function StationBadge({ station, color, size = 'md' }: { station: StationInfo; color: string; size?: 'sm' | 'md' }) {
  const dims = size === 'sm' ? 'w-7 h-5 text-[10px]' : 'w-9 h-6 text-xs';
  return (
    <span
      className={`${dims} rounded-md flex items-center justify-center font-extrabold text-white shrink-0 tracking-wide`}
...
// Custom home-station picker — mirrors LinePicker's button + dropdown style,
// showing each station's code badge in the line's colour.
function StationPicker({
  ...
  const station = getStation(line.id, stationCode);
  ...
  return (
    <div className="relative flex-1" ref={rootRef}>
      ...
      >
        <StationBadge station={station} color={line.color} size="sm" />
        <span className="truncate">{getStationName(station.code, station.name, lang)}</span>
      </button>
      ...
      {open && (
        <div className="absolute z-20 left-0 right-0 mt-1 bg-go-dark border border-white/20 rounded-lg shadow-xl overflow-hidden max-h-72 overflow-y-auto">
          {line.homeStations.map((s) => (
            <button
              key={s.code}
              type="button"
              onClick={() => { onChange(s.code); setOpen(false); }}
              className={`w-full flex items-center gap-2 px-2 py-2 text-left text-sm text-white hover:bg-white/10 ${s.code === stationCode ? 'bg-white/10' : ''}`}
            >
              <StationBadge station={s} color={line.color} size="sm" />
              <span className="truncate">{getStationName(s.code, s.name, lang)}</span>
            </button>
          ))}
        </div>
      )}
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col max-w-md mx-auto">
      ...
      <header ref={headerRef} className="sticky top-0 z-20 bg-go-dark text-white shadow-lg">
        ...
        <div className="flex items-center gap-3 px-4 pt-4 pb-2">
          ...
          >
            {alertsLoading ? (
              <BellIcon className="w-7 h-7 text-white/70" />
            ) : (
              <LineStatusIcon status={lineStatus} className="w-7 h-7" />
            )}
          </button>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            >
              <svg className="w-5 h-5 text-white/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M12 3v13m0-13l-4 4m4-4l4 4" />
              </svg>
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mx-4 mb-2">
          ...
          <StationPicker line={line} stationCode={homeStationCode} lang={lang} onChange={handleHomeStationChange} />
        </div>
        ...
        {trips.length === 0 ? (
          ...
          <>
            ...
            <div className="relative mt-4 mb-2 mx-1">
              <div className="flex items-stretch rounded-xl bg-white border border-gray-100 shadow-sm overflow-hidden">
                <button
                  onClick={() => setShowDataSourceInfo((v) => !v)}
                  aria-label="Show data source info"
                  className="flex-1 text-left px-4 py-3 hover:bg-gray-50 active:bg-gray-100 transition-colors focus:outline-none"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center px-3 text-xs font-medium text-gray-400 hover:text-go-green transition-colors shrink-0"
                >
                  {t('official')}
                </a>
              </div>
